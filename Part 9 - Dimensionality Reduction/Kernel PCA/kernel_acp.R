...
training_set_pca$Purchased = training_set[,3]

testing_set_pca =  as.data.frame(predict(kpca,testing_set))
testing_set_pca$Purchased = testing_set[,3]
...
X1 = seq(min(set[, 1]) - 1, max(set[, 1]) + 1, by = 0.06)
X2 = seq(min(set[, 2]) - 1, max(set[, 2]) + 1, by = 0.06)
